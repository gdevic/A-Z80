...
if (pla[17] && !pla[50]) begin
    ...
    if (M2 && T2) begin  fMRead=1;
                    ...
                    ctl_bus_inc_we=1; ctl_inc_cy=1; /* Output enable while holding to increment */ end
...
if (pla[5]) begin
    ...
    if (M1 && T6) begin  nextM=1; setM1=1;
                    ...
                    ctl_bus_inc_we=1; /* Output enable incrementer to the abus */ end
end
...
if (pla[23] && pla[16]) begin
    ...
    if (M2 && T2) begin  fMWrite=1;
                    ctl_reg_gp_we=1; ctl_reg_gp_sel=`GP_REG_AF; ctl_reg_gp_hilo=2'b11; ctl_reg_use_sp=1; ctl_sw_4u=1; /* Write 16-bit SP, enable SW4 upstream */
                    ctl_bus_inc_we=1; ctl_inc_cy=1; ctl_inc_dec=1; /* Output enable while holding to decrement */ end
...
if (pla[9]) begin
    ...
    if (M1 && T6) begin  nextM=1; setM1=1;
                    ...
                    ctl_bus_inc_we=1; ctl_inc_cy=1; ctl_inc_dec=op3; /* Used for INC/DEC: decrement if op3 is set */ end
end
...
if (pla[25]) begin
    ...
    if (M1 && T1) begin  fFetch=1;
                    ...
                    ctl_alu_oe=1; /* Enable ALU onto the data bus */
                    ctl_alu_res_oe=1; /* Result latch */
                    ctl_alu_core_R=1; ctl_alu_core_V=1; ctl_alu_core_S=1; ctl_pf_sel=`PFSEL_P;
                    ctl_alu_sel_op2_high=1; /* Activate ALU operation on high nibble */
                    ctl_flags_xy_we=1;
                    ctl_flags_hf_we=1;
                    ctl_flags_nf_we=1; ctl_flags_nf_set=0;
...
if (pla[15] && op3) begin
    ...
    if (M1 && T1) begin  fFetch=1;
                    ...
                    ctl_flags_xy_we=1;
                    ctl_flags_pf_we=1;
                    ctl_flags_nf_we=1; ctl_flags_nf_set=0; end
    if (M1 && T2) begin  fFetch=1;
                    ctl_reg_gp_we=1; ctl_reg_gp_sel=`GP_REG_AF; ctl_reg_gp_hilo=2'b01;
                    ctl_sw_2u=1;
                    ctl_flags_oe=1; /* Enable FLAGT onto the data bus */ end
...
if (pla[72] && !pla[55]) begin
    ...
    if (M1 && T1) begin  fFetch=1;
                    ...
                    ctl_alu_oe=1; /* Enable ALU onto the data bus */
                    ctl_alu_res_oe=1; /* Result latch */
                    ctl_alu_core_R=0; ctl_alu_core_V=0; ctl_alu_core_S=1; ctl_pf_sel=`PFSEL_P;
                    ctl_alu_sel_op2_high=1; /* Activate ALU operation on high nibble */
                    ctl_flags_sz_we=1;
                    ctl_flags_xy_we=1;
                    ctl_flags_hf_we=1;
...
if (pla[73] && !pla[55]) begin
    ...
    if (M1 && T1) begin  fFetch=1;
                    ...
                    ctl_alu_oe=1; /* Enable ALU onto the data bus */
                    ctl_alu_res_oe=1; /* Result latch */
                    ctl_alu_sel_op2_neg=1; ctl_alu_core_R=0; ctl_alu_core_V=0; ctl_alu_core_S=1; ctl_pf_sel=`PFSEL_P;
                    ctl_alu_sel_op2_high=1; /* Activate ALU operation on high nibble */
                    ctl_flags_sz_we=1;
                    ctl_flags_xy_we=1;
                    ctl_flags_hf_we=1;
...
if (pla[85]) begin
    ...
    begin 
                    ctl_alu_core_R=0; ctl_alu_core_V=0; ctl_alu_core_S=1; ctl_pf_sel=`PFSEL_P; end
end

if (pla[86]) begin
    $display("pla[86] : ALU OR");
    begin 
                    ctl_alu_core_R=1; ctl_alu_core_V=1; ctl_alu_core_S=1; ctl_pf_sel=`PFSEL_P; end
end
...
if (M1) begin
    ...
    if (M1 && T4) begin  fFetch=1;
                    ...
                    ctl_bus_inc_we=1; ctl_inc_cy=1; /* Output enable while holding to increment */
