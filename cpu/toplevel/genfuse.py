...
# Number of tests to run, use -1 to run all
run_tests = 1
...
def RegIn(reg, hex):
    ftest.write("   force dut.reg_file.b2v_latch_" + reg + "_lo.oen=1;\n")
    ftest.write("   force dut.reg_file.b2v_latch_" + reg + "_hi.oen=1;\n")
    ftest.write("   if (dut.reg_file.b2v_latch_" + reg + "_lo.db!=8'h" + hex[2:] + ") $display(\"Reg " + reg + " mismatch: " + hex[2:] + "\");\n")
    ftest.write("   if (dut.reg_file.b2v_latch_" + reg + "_hi.db!=8'h" + hex[0:2] + ") $display(\"Reg " + reg + " mismatch: " + hex[0:2] + "\");\n")
    ftest.write("#1 release dut.reg_file.b2v_latch_" + reg + "_lo.we;\n")
    ftest.write("   release dut.reg_file.b2v_latch_" + reg + "_hi.we;\n")
...
while True:
    ...
    while True:
        ...
        while True:
            ...
            if d=="-1":
                break
            ftest.write("   force ram.Mem[" + str(address) + "] = 8'h" + d + ";\n")
            address = address+1

    ftest.write("#1 force dut.z80_top.fpga_reset=0;\n")
    ftest.write("   $display(\"Releasing reset at %d\", $time);\n")
